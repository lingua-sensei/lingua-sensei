...
def audio_file_size(f: FieldFile):
    if f.size > MAX_AUDIO_FILE_SIZE:
        raise ValidationError(f"File too large, must not exceed 4 MB")
...
class Audio(models.Model):
    ...
    file = models.FileField(upload_to="audios/", validators=[
    ...
    correspondent_language = models.CharField(
        max_length=2,
        choices=LANGUAGES,
        blank=True, null=True # Optional for linked audios
    )
