    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "sentence":
            return queryset.filter(
                Q(linked_count__gt=0) & ~Q(correspondent_text__isnull=True) & ~Q(correspondent_text="") & ~Q(correspondent_language__isnull=True) & ~Q(correspondent_language="")
            )
...
class AudioAdmin(admin.ModelAdmin):
    ...
    def audio_player(self, obj):
        if obj.file:
            return format_html('<audio controls><source src="{}" type="audio/mpeg">Your browser does not support the audio element.</audio>', obj.file.url)
    ...
    def get_queryset(self, request):
        ...
        queryset = queryset.annotate(linked_audio_count=Count("linked_audios"))
        return queryset
    ...
    def is_sentence(self, obj):
        ...
        is_single_word = " " in obj.text
        return has_links and has_correspondent_info and not is_single_word
